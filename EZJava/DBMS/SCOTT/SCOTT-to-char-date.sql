...
-- NUMBER + 문자열(숫자형태가 아닌 경우) -> INVALID NUMBER오류
SELECT empno, 
ename,
empno + 'HELLO'
FROM emp
...
-- 문자타입 + 문자열 -> invaild number오류
-- plus(+) 로 인해서 숫자 연산으로 인식하여 오류 발생
SELECT empno, 
ename,
ename + 'HELLO'
FROM emp
...
SELECT sysdate, to_char(sysdate, 'DDD') FROM dual; --일(1년 중 며칠, 1~365)
